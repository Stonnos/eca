...
1) configStorageType - тип хранения настроек приложения (FILE - в файловой системе, IN_MEMORY - в памяти).
...
17) memoryThresholdForGC - объем памяти в байтах, при превышении которого необходимо запустить очистку памяти (GC)
18) memoryMonitoringPeriodMinutes - значение интервала в мин. между запусками процесса очистки памяти
...
13) authToken - персональный токен доступа к API
...
2. После сборки проекта в директории eca-gui/target будут лежать два дистрибутива eca-7.0.zip (для windows), eca-7.0.tar.gz (для linux)
...
mvn versions:set -D newVersion=version -D generateBackupPoms=false